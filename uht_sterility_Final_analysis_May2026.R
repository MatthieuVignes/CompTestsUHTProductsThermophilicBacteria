...
plot_signal <- function(df, test_col, ylabel, hlines = NULL, hline_labels = NULL,
  ...
  p <- df |>
    ...
    labs(title = paste(prod_name, "—", ylabel),
         x = "Time (h)", y = paste0("log₁₀(", ylabel, ")"), colour = "Bacteria") +
    theme_bw(base_size = 10) +
    theme(legend.position = "right")
...
#
# Three spike level groups (analysed separately, never pooled):
#   ">10"  : inoculated replicates where PlateRaw > 10 CFU/mL (high positives)
#   "0-10" : inoculated replicates where PlateRaw <= 10 CFU/mL (fractional)
#   "0"    : blank replicates (PlatePos always 0; blanks pooled across time)
#
# Bacteria groups:
#   Geobacillus     = Geo 1, Geo 2, Geo 3, Geo 4
#   Anoxybacillus   = AF 5, AF 6, AF 7, AF 8, AF 5 Hi, AF 8 Hi
#   B. licheniformis = BL 1, BL 2  (Chocolate only)
#
# CI method:
...
compute_pod_table_24h <- function(inoculated_df, blanks_df, prod_name,
  ...
  inoc_24 <- inoculated_df |>
    ...
    mutate(
      ...
      spike_grp  = if_else(PlatePos == 1, ">10", "0-10"),
      test_pos   = !is.na(log10s(.data[[test_col]])) &
        log10s(.data[[test_col]]) > log_thr
    )
  
  # Blanks: pool across all time-points for this product
  bl <- blanks_df |>
    mutate(
      bact_grp  = bact_group(as.character(Bact)),
      spike_grp = "0",
  ...
  combined <- bind_rows(inoc_24, bl)
  
  # Compute all CI quantities via pmap_dfr to avoid rowwise() list-column
  # subscripting issues (named vectors inside list() cannot be reliably
  # extracted with [[name]] inside rowwise mutate in all dplyr versions).
  counts <- combined |>
    group_by(bact_grp, spike_grp) |>
    summarise(
      N      = n(),
      X_test = sum(test_pos,    na.rm = TRUE),
      X_ref  = sum(PlatePos == 1, na.rm = TRUE),
      .groups = "drop"
    )
  ...
  ci_rows |>
    mutate(
      ...
      across(c(POD_test, CI_test_lo, CI_test_hi,
               POD_ref,  CI_ref_lo,  CI_ref_hi,
               dPOD, dPOD_lo, dPOD_hi), ~ round(.x, 2))
    ) |>
    select(Product, Test, Threshold_log10,
           Isolate = bact_grp,
...
cat("Master best-threshold CSV written\n")
...
cat("Master avg performance CSV written\n")
